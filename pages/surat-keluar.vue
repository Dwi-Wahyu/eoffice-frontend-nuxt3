<template>
  <WidgetsCardBaseCard add-class="mt-16 mb-10 w-full h-fit flex-col">
    <WidgetsCardBaseCard
    ...
    <div class="mt-4 shadow rounded-lg">
      ...
      <div class="border border-b-0 p-4">
        <WidgetsDatatableBaseTable
          :headers="suratKeluarStore.tableHeaders"
          :data="suratKeluarStore.data"
          :loading="suratKeluarStore.loading"
      ...
      >
        ...
        <WidgetsDatatableBasePagination
          :model-value="perPage"
          :current-page="suratKeluarStore.currentPage"
          :total-pages="suratKeluarStore.totalPages"
          :total-datas="suratKeluarStore.totalDatas"
          :per-page="suratKeluarStore.perPage"
...
import { useMySuratKeluarStore } from "~/store/surat-keluar";
...
const suratKeluarStore = useMySuratKeluarStore();
...
const { totalDatas, perPage } = storeToRefs(suratKeluarStore);
...
const loadData = () => {
  const payload = {
    search: search.value,
    per_page: suratKeluarStore.perPage,
  };
  suratKeluarStore.getData(payload).then((res: any) => {});
};

const handlePageChange = (page: any) => {
  suratKeluarStore.currentPage = page;
  loadData();
};
...
const handlePerPageChange = (value: any) => {
  suratKeluarStore.perPage = value;
  loadData();
};
